...
Library    ../support/gmail_reader.py
Library    Process
...
*** Variables ***
${API}               https://qa.renew.api.parmple.com
${bizRegNo_DIR}      assets/bizRegNo
${testfile_DIR}      assets/testfile
${testfile_PATH}    assets/testfile/automation_sample_attachment.pdf

${MAX_RETRY}         5

${EMAIL}             chera.workspace@gmail.com
...
Find Valid Biz Number
    ...
    Log To Console    \n[선택된 파일] ${file}
    Log To Console    [선택된 사업자 번호] ${bizRegNo}
...
---- Testcase (로그인)
    ...
    Sleep    1
    ${abs_path}=    Normalize Path    ${CURDIR}/${testfile_PATH}
    ...
    ${result}=    Run Process    python    support/gmail_reader.py    stdout=PIPE
    ...
    Input Password    id=password    ${USER_RN_PW}
    Sleep    1
    Input Password    id=passwordCheck    ${USER_RN_PW}
    Sleep    1
